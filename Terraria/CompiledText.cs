...
public sealed class CompiledText
{
	...
	public CompiledText(string text, int wrapWidth, Style style, MarkupType markupType = MarkupType.Html)
	{
		...
		while (i < text.Length)
		{
			...
			{
				i += 2;
				skipTag(text, markupType, ref i, closingTag: true, out var tagType, out var _);
			...
			{
				i++;
				skipTag(text, markupType, ref i, closingTag: false, out var tagType2, out var newColor2);
