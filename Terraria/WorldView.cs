...
namespace Terraria
{
	public class WorldView : IDisposable
	{
		...
		{
			float num = (time.tileColorf.X + time.tileColorf.Y + time.tileColorf.Z) * (142f / (339f * (float)Math.PI));
		...
		private unsafe void DrawBackground()
		{
			...
			float num5 = (float)(screenPosition.Y - (Main.worldSurface << 4)) * 0.00333333341f;
			if (num5 < 0f)
			{
				num5 = 0f;
			}
			...
			float num7 = (float)(screenPosition.Y - 270 + 200 - (Main.rockLayer << 4)) * 0.00333333341f;
			if (num7 < 0f)
			{
				num7 = 0f;
			}
		...
		private unsafe void DrawNonSolidTiles()
		{
			...
			fixed (Tile* ptr = Main.tile)
			{
				...
				if (!player.dead)
				{
					Tile* ptr2 = (Tile*)(long)(((Main.frameCounter & 0x20) == 0) ? ((IntPtr)(void*)null) : ((IntPtr)(ptr + (player.tileInteractX * 1440 + player.tileInteractY))));
							...
							if (!Main.tileSolid[num7])
							{
								...
								case 61:
									if (ptr4->frameX == 144)
									{
										...
										{
											Dust* ptr7 = dustLocal.NewDust(i * 16, num6 * 16, 16, 16, 44, 0.0, 0.0, 250, default(Color), 0.40000000596046448);
											if (ptr7 != null)
											{
												ptr7->fadeIn = 0.7f;
											}
								...
								else if (newColor.R > 1 || newColor.G > 1 || newColor.B > 1)
								{
									...
									else
									{
										...
										if (SMOOTH_LIGHT && Main.tileSolid[num7] && num7 != 137)
										{
											...
											else if (newColor.R > 38 || (float)(int)newColor.G > 41.8f || (float)(int)newColor.B > 45.6000023f)
											{
												s.Width = 8;
												s.Height = 8;
				...
				if (!player.dead)
				{
					...
					for (i = -3; i < 23; i++)
					{
						...
						for (int k = -4; k < 20; k++)
						{
							ptr13->flags &= ~(Tile.Flags.NEARBY | Tile.Flags.SELECTED);
							ptr13++;
						}
					}
				}
									...
									if (!Main.tileSolidTop[type] && (ptr2[-1].liquid > 0 || ptr2[1].liquid > 0 || ptr2[-1440].liquid > 0 || ptr2[1440].liquid > 0))
									{
										...
										if (!flag5 || num7 != 1)
										{
											...
											if ((!flag3 || (!flag && !flag2)) && (!flag4 || !flag3))
											{
												...
												{
													int num8 = (int)((float)(256 - num6) * 0.03125f) << 1;
													pos2.Y += num8;
													s2.Height -= num8;
													if (!flag || !flag2)
													{
