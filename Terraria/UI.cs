...
public sealed class UI
{
	...
	public const float DEAD_ZONE_SQUARED = 1f / 64f;

	public const float WORLD_FADE_START = -0.25f;

	private const float WORLD_FADE_SPEED = 1f / 30f;

	private const float UI_FADE_SPEED = 1f / 30f;
	...
	public float[] hotbarScale = new float[10] { 1f, 0.75f, 0.75f, 0.75f, 0.75f, 0.75f, 0.75f, 0.75f, 0.75f, 0.75f };

	private static float inventoryScale;

	public float[] inventoryMenuSectionScale = new float[5] { 0.75f, 0.75f, 1f, 0.75f, 0.75f };
	...
	private void UpdateMenu()
	{
		...
		else if (this.menuMode == MenuMode.SIGN_IN)
		{
			if (!Guide.IsVisible)
			{
				if (this.signedInGamer == null)
				{
					...
					{
						if (signedInGamer.PlayerIndex == controller)
						{
				...
				if (this.signedInGamer != null)
				{
					if (!IsGamerValid())
					{
						MessageBox.Show(controller, Lang.menu[5], Lang.inter[43], new string[1] { Lang.menu[90] });
		...
		else if (this.menuMode == MenuMode.UPSELL)
		{
			...
			else if (IsButtonTriggered(Buttons.X) && this.signedInGamer.Privileges.AllowPurchaseContent && !Guide.IsVisible)
			{
				bool flag4;
				do
				{
	...
	private void DrawHud()
	{
		...
		for (int k = 0; k < 10; k++)
		{
			...
			{
				int num17 = player.buff[k].Time / 60;
				Main.strBuilder.Length = 0;
	...
	public UserString GetInputText(UserString oldString, string title = null, string desc = null, bool validate = true)
	{
		if (!inputTextEnter && oldString.IsEditable())
		{
			...
			else if (kbResult != null && kbResult.IsCompleted)
			{
				inputTextEnter = true;
	...
	private void UpdateMouse()
	{
		...
		if (uiDelay > 0)
		{
			if (gpState.ThumbSticks.Left.LengthSquared() <= 1f / 64f && gpState.ThumbSticks.Right.LengthSquared() <= 1f / 64f && gpState.DPad.Up == ButtonState.Released && gpState.DPad.Down == ButtonState.Released && gpState.DPad.Left == ButtonState.Released && gpState.DPad.Right == ButtonState.Released)
			{
				uiDelay = 0;
				uiDelayValue = 12;
			}
	...
	public void CheckPlayerStorage(string name)
	{
		...
		if (isTransferredFromOtherPlayer)
		{
			...
			{
				MessageBox.Show(controller, Lang.menu[9], string.Format(Lang.inter[78], name), new string[1] { Lang.menu[90] });
	...
	private void DeviceDisconnected(object sender, EventArgs args)
	{
		...
		MessageBox.Show(controller, Lang.menu[69], Lang.menu[70], new string[1] { Lang.menu[90] });
	...
	private void DeviceSelectorCanceled(object sender, EventArgs args)
	{
		LoadPlayers();
		WorldSelect.LoadWorlds();
		MessageBox.Show(controller, Lang.menu[69], Lang.menu[66], new string[1] { Lang.menu[90] });
	}
	...
	private void DeviceSelected(object sender, EventArgs e)
	{
		...
		if (transferredPlayerStorage.Count <= 0)
		{
			if (!OpenSettings())
			{
				MessageBox.Show(controller, Lang.menu[9], Lang.menu[102], new string[1] { Lang.menu[90] });
			}
			LoadPlayers();
			if (this == main && !WorldSelect.LoadWorlds())
			{
				MessageBox.Show(controller, Lang.menu[9], Lang.menu[103], new string[1] { Lang.menu[90] });
			}
		}
	}
					...
					while (true)
					{
						if (num2 < 5)
						{
							if (num2 < num)
							{
								...
								{
									MessageBox.Show(controller, Lang.menu[9], Lang.menu[12], new string[1] { Lang.menu[90] });
									goto IL_0017;
								}
							}
							else
	...
	private string nextLoadPlayer()
	{
		...
		if (HasPlayerStorage())
		{
			try
			{
				...
				while (storageContainer.FileExists(text));
			}
			catch (IOException)
			{
				ReadError();
	...
	private void DrawControlsInventory()
	{
		...
		else
		{
			...
			{
				flag = inventoryEquipY switch
				{
					0 => mouseItem.headSlot >= 0, 
					1 => mouseItem.bodySlot >= 0, 
					2 => mouseItem.legSlot >= 0, 
					_ => mouseItem.accessory, 
				};
			}
	...
	private void HelpText()
	{
		...
		for (int i = 0; i < 48; i++)
		{
			...
			switch ((Item.ID)player.inventory[i].type)
			{
	...
	public void UpdateNpcChat()
	{
		...
		focusColor = new Color(num, (int)((double)num * 0.9090909090909091), num >> 1, num);
	...
	private void Reforge(int slot, bool isArmor = false)
	{
		if (toolTip.Prefix(-3) && player.BuyItem(toolTip.value))
		{
			...
			if (isArmor)
			{
				ref Item reference = ref player.armor[slot];
				reference = toolTip;
			}
			else
			{
				ref Item reference2 = ref player.inventory[slot];
				reference2 = toolTip;
			}
	...
	private void UpdateInventory()
	{
		...
		if (IsButtonTriggered(Buttons.A))
		{
			...
			else
			{
				...
				if (mouseItem.type > 0 && item2.type > 0 && !flag2 && mouseItemSrcSection == InventorySection.ITEMS && mouseItemSrcX < 10 && mouseItemSrcY < 4)
				{
					...
					{
						ref Item reference = ref player.inventory[num4];
						reference = mouseItem;
						mouseItem.Init();
					}
		...
		else if (gpState.IsButtonDown(Buttons.RightTrigger))
		{
			if (gpPrevState.IsButtonUp(Buttons.RightTrigger))
			{
				...
				{
					ref Item reference2 = ref player.inventory[num2];
					reference2 = player.armorSwap(ref player.inventory[num2]);
				}
			}
	...
	private void DrawInventory(int itemsSectionX, int itemsSectionY)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				int x = itemsSectionX + (int)((float)i * 52.149998f);
				int y = itemsSectionY + (int)((float)j * 52.149998f);
				int num3 = i + j * 10;
		...
		for (int k = 0; k < 4; k++)
		{
			int x2 = (int)((float)(6 + k) * 52.149998f) + itemsSectionX;
		...
		for (int l = 0; l < 4; l++)
		{
			int x3 = (int)((float)(6 + l) * 52.149998f) + itemsSectionX;
	...
	private void UpdateStorage()
	{
		...
		{
			ref Item reference = ref chest2.item[num];
			reference = player.armorSwap(ref chest2.item[num]);
			if (chest >= 0)
			{
				NetMessage.CreateMessage2(32, chest, num);
				NetMessage.SendMessage();
	...
	private void UpdateEquip()
	{
		...
		{
			ref Item reference = ref player.armor[num2];
			reference = player.armorSwap(ref player.armor[num2]);
		}
	}
	...
	private void DrawEquip(int INVENTORY_X, int INVENTORY_Y)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			{
				DrawInventoryItem(itemColor: ((!craftGuide || player.armor[j].material) && (!reforge || player.armor[j].Prefix(-3))) ? new Color(255, 255, 255, 255) : DISABLED_COLOR, item: ref player.armor[j], x: num, y: y);
			}
			y += 56;
		}
		y += 32;
		for (int k = 3; k < 8; k++)
		{
			...
			{
				DrawInventoryItem(itemColor: ((!craftGuide || player.armor[k].material) && (!reforge || player.armor[k].Prefix(-3))) ? new Color(255, 255, 255, 255) : DISABLED_COLOR, item: ref player.armor[k], x: num, y: y);
			}
			num += 56;
		}
		num -= 56;
		...
		for (int l = 8; l < 11; l++)
		{
			...
			{
				DrawInventoryItem(itemColor: ((!craftGuide || player.armor[l].material) && (!reforge || player.armor[l].Prefix(-3))) ? new Color(255, 255, 255, 255) : DISABLED_COLOR, item: ref player.armor[l], x: num, y: y);
			}
			y += 56;
		}
	...
	private void DrawInventoryMenu()
	{
		...
		Main.DrawRect(inventorySection switch
		{
			InventorySection.CRAFTING => 441, 
			InventorySection.EQUIP => 442, 
			InventorySection.CHEST => flag2 ? 445 : 444, 
			InventorySection.HOUSING => 449, 
			_ => 451, 
		}, new Rectangle(num, sAFE_AREA_OFFSET_T, 864, 446), 96);
		Color c = default(Color);
		int num2 = num + (864 - ((flag3 ? 4 : 3) * 43 + 56)) / 2;
		for (int i = 0; i < 5; i++)
		{
			int itemTexId;
			switch ((InventorySection)(byte)i)
			{
			...
			case InventorySection.CHEST:
				if (flag2)
				{
					itemTexId = Chest.GetShopOwnerHeadTextureId(npcShop);
					break;
				}
				if (!flag)
				{
			...
			DrawInventoryItem(itemTexId, num2, y, itemColor);
			num2 += (int)(52f * num4) + 4;
		}
		string text;
		...
		if (count > 0)
		{
						...
						for (int l = -num11; l < 8 + num11; l++)
						{
							...
							if (player.recipesNew.Get(num15))
							{
								...
								else
								{
									...
									SpriteSheet<_sheetTiles>.Draw(23, num12 + 28 - 12, num7 + 28 - 8, color, (float)((double)Main.frameCounter * (1.0 / (4.0 * Math.PI))), (float)(0.7 + Math.Sin((double)Main.frameCounter * (1.0 / (32.0 * Math.PI))) * 0.3));
								}
							}
							num12 += 56;
						}
			...
			for (int m = 0; m < 3; m++)
			{
				...
				while (num19 < 4)
				{
					...
					{
						int id2 = 442;
						color = ((num18 >= craftingRecipe.numRequiredItems) ? new Color(64, 64, 64, 64) : color2);
	...
	private void DrawMiniMap()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			{
				DrawInventoryCursor(num4, num5, 0.6153846153846154);
			}
			else
			{
				SpriteSheet<_sheetSprites>.DrawScaledTL(451, num4, num5, Color.White, 0.61538464f);
			}
			num4 += 8;
			num5 += 8;
		...
		for (int num8 = 8; num8 > 0; num8--)
		{
			...
			{
				DrawInventoryCursor(num6 - 2, num9 - 2, 0.9230769230769231);
			}
			else
			{
		...
		if (toolTip.type > 0)
		{
			...
			else
			{
				...
				if (toolTip.buffTime > 0)
				{
					...
					{
						Main.strBuilder.Append(((int)Math.Round((double)(toolTip.buffTime / 60) / 60.0)).ToStringLookup());
	...
	public void PositionMapScreenCursor(int dx, int dy)
	{
		if (IsButtonTriggered(Buttons.A))
		{
			...
			else
			{
				...
				{
					teamSelected = (byte)((mapScreenCursorX != 1) ? ((uint)(mapScreenCursorX + 1)) : 0u);
				}
			}
			return;
		}
	...
	public bool TestStorageSpace(string container, string destinationPath, int writeSize)
	{
		...
		MessageBox.Show(controller, Lang.menu[5], Lang.inter[70], new string[1] { Lang.menu[90] });
		return false;
	}

	public void WriteError()
	{
		MessageBox.Show(controller, Lang.menu[5], Lang.gen[54], new string[1] { Lang.menu[90] });
	}

	public void ReadError()
	{
		MessageBox.Show(controller, Lang.menu[9], Lang.gen[53], new string[1] { Lang.menu[90] });
	}
	...
	public void CheckHDTV()
	{
		...
		{
			Main.isHDTV = true;
			MessageBox.Show(controller, Lang.menu[3], Lang.inter[71], new string[1] { Lang.menu[90] });
		}
	}
	...
	public void CheckUserGeneratedContent()
	{
		if (!IsUserGeneratedContentAllowed())
		{
			...
			MessageBox.Show(options: (!autoSave && IsStorageEnabledForAnyPlayer()) ? new string[2]
			{
				Lang.inter[2],
				Lang.inter[1]
			} : new string[1] { Lang.menu[15] }, controller: controller, caption: Lang.menu[3], contents: Lang.inter[79], autoUpdate: false);
