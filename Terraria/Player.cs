...
public sealed class Player
{
	...
	{
		20, 42, 0, -16, 0, -16, -16, 32, 0, -16,
		0, -16, -16, 32, 0, -16, 0, -16, 48, 32,
		0, -16, 0, -16, 0, -16, -16, 0, -16, 0,
		-16, 0, -16, 0, 0, 16, 0, 16, 0, 16,
	...
	public void dropItemCheck()
	{
		...
		if (noThrow == 0 && ((controlThrow && inventory[selectedItem].type > 0) || ((ui.inventoryMode == 0 || ui.IsButtonUntriggered(Buttons.X)) && ui.mouseItem.type > 0 && ui.mouseItem.stack > 0)))
		{
			...
			else
			{
				...
				ref Item reference2 = ref Main.item[num];
	...
	public void QuickMana()
	{
		...
		for (int i = 0; i < 48; i++)
		{
			...
			Main.PlaySound(2, aabb.X, aabb.Y, inventory[i].useSound);
			...
			if (isLocal())
			{
				if (NPC.wof >= 0 && Main.npc[NPC.wof].active != 0)
				{
					...
					if (tongued)
					{
						...
						float num44 = Main.npc[NPC.wof].position.X + (float)(Main.npc[NPC.wof].width / 2) - vector2.X;
						float num45 = Main.npc[NPC.wof].position.Y + (float)(Main.npc[NPC.wof].height / 2) - vector2.Y;
	...
	public unsafe void PlayerFrame()
	{
		...
		else
		{
			...
			else if (head == 8 && body == 8 && legs == 8)
			{
				...
				{
					Dust* ptr2 = Main.dust.NewDust((int)(position.X - velocity.X * 2f), (int)(position.Y - velocity.Y * 2f) - 2, 20, 42, 40, 0.0, 0.0, 50, default(Color), 1.399999976158142);
					if (ptr2 != null)
					{
						ptr2->noGravity = true;
						ptr2->velocity *= 0.25f;
	...
	public unsafe void ItemCheck(int i)
	{
		fixed (Item* ptr = &inventory[selectedItem])
		{
			...
			if ((ptr->type == 8 || ptr->type == 523 || (ptr->type >= 427 && ptr->type <= 433)) && !wet)
			{
				...
				Vector3 rgb = num14 switch
				{
					1 => new Vector3(0f, 0.1f, 1.3f), 
					2 => new Vector3(1f, 0.1f, 0.1f), 
					3 => new Vector3(0f, 1f, 0.1f), 
					4 => new Vector3(0.9f, 0f, 0.9f), 
					5 => new Vector3(1.3f, 1.3f, 1.3f), 
					6 => new Vector3(0.9f, 0.9f, 0f), 
					7 => new Vector3(0.5f * Main.demonTorch + 1f * (1f - Main.demonTorch), 0.3f, Main.demonTorch + 0.5f * (1f - Main.demonTorch)), 
					8 => new Vector3(0.85f, 1f, 0.7f), 
					_ => new Vector3(1f, 0.95f, 0.8f), 
				};
				int num15 = num14;
				num15 = num15 switch
				{
							...
							for (int num51 = tileTargetX - 1; num51 <= tileTargetX + 1; num51++)
							{
								for (int num52 = tileTargetY - 1; num52 <= tileTargetY + 1; num52++)
								{
									if (num50 < 256 && Main.tile[num51, num52].lava == lava)
									{
										...
										Main.tile[num51, num52].lava = (byte)((Main.tile[num51, num52].liquid != 0) ? ((uint)lava2) : 0u);
			...
			if (ptr->damage >= 0 && ptr->type > 0 && !ptr->noMelee && itemAnimation > 0)
			{
				...
				if (!flag4)
				{
					...
					else if (ptr->type == 273)
					{
						if (Main.rand.Next(6) == 0)
						{
							Main.dust.NewDust(rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height, 14, direction * 2, 0.0, 150, default(Color), 1.399999976158142);
						}
					...
					{
						Dust* ptr5 = Main.dust.NewDust(rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height, 6, velocity.X * 0.2f + (float)(direction * 3), velocity.Y * 0.2f, 100, default(Color), 1.899999976158142);
						if (ptr5 != null)
						{
							ptr5->noGravity = true;
						}
					...
					else if (ptr->type == 367 || ptr->type == 368)
					{
						if (Main.rand.Next(4) == 0)
						{
							Dust* ptr7 = Main.dust.NewDust(rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height, 57, velocity.X * 0.2f + (float)(direction * 3), velocity.Y * 0.2f, 100, default(Color), 1.100000023841858);
							if (ptr7 != null)
							{
								ptr7->noGravity = true;
			...
			if (ptr->type == 50 && itemAnimation > 0)
			{
				...
				{
					Main.dust.NewDust(15, ref aabb, 0.0, 0.0, 150, default(Color), 1.100000023841858);
				}
			}
			if (!isLocal())
			{
	...
	public Color GetImmuneAlpha(Color newColor)
	{
		...
		double num = (double)(255 - immuneAlpha) * (1.0 / 255.0);
		if (shadow > 0f)
		{
	...
	public Player DeepCopy()
	{
		...
		for (int l = 0; l < 20; l++)
		{
			ref Item reference4 = ref player.bank.item[l];
			reference4 = bank.item[l];
			ref Item reference5 = ref player.safe.item[l];
			reference5 = safe.item[l];
		}
